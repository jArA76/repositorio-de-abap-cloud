...
CLASS zcl_jugador_profesional_16 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  INHERITING FROM zcl_jugador_base_16.
  ...
  PUBLIC SECTION.
    METHODS:
      ...
      zif_puntuable_16~calcular_puntos
        REDEFINITION,

      zif_puntuable_16~get_resumen
        REDEFINITION.
...
CLASS zcl_jugador_profesional_16 IMPLEMENTATION.
  ...
  METHOD zif_puntuable_16~calcular_puntos.
    rv_puntos = mv_victorias * mc_puntos_victoria.
  ENDMETHOD.

  METHOD zif_puntuable_16~get_resumen.
    rv_texto = |PRO { mv_nombre } ({ mv_pais }) | &&
              |V:{ mv_victorias } D:{ mv_derrotas } | &&
              |Puntos:{ zif_puntuable_16~calcular_puntos( ) }|.
  ENDMETHOD.
