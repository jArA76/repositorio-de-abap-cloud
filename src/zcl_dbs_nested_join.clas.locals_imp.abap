...
*      INTO TABLE @DATA(keys).
    ...
    FIELDS carrier_id, connection_id, flight_date,
           ...
           price, currency_code, seats_max - seats_occupied as seats_free
...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    FIELDS
    "name,
    carrier_id && ' ' && name as id_name,
     currency_code
     WHERE carrier_id = @i_carrier_id INTO ( @me->name, @me->currency_code ).
     "nuevo codigo

    "codigo antiguo INTO ( @me->name, @me->currency_code ).
